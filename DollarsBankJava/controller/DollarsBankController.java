package DollarsBankJava.controller;
...
public class DollarsBankController {
    ...
    public DollarsBankController() {
        ...
        for (int i = 0; i < FileStorage.address.length; i++) {
            Customer c = new Customer();
            c.setName(FileStorage.u_name[i]);
            c.setAddress(FileStorage.address[i]);
            c.setContact_number(FileStorage.phone[i]);
            c.setUser_id(FileStorage.uid[i]);
            userList.add(c);
        }

        accList = new ArrayList<Account>();
        for (int i = 0; i < FileStorage.uid.length; i++) {
            accList.add(new Account(FileStorage.uid[i], FileStorage.pass[i],
                    FileStorage.bal[i]));
        }
    }
    ...
    public boolean checkPassValid(String pw) {
